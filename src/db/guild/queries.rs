...
use super::models::{CreateGuild, Guild, GuildMember};
use crate::db::DbPool;
use chrono::Utc;
use uuid::Uuid;

/// Get a guild by ID
pub async fn get_guild_by_id(pool: &DbPool, guild_id: Uuid) -> Result<Option<Guild>, sqlx::Error> {
...
/// Get a guild by name
pub async fn get_guild_by_name(pool: &DbPool, name: &str) -> Result<Option<Guild>, sqlx::Error> {
    sqlx::query_as::<_, Guild>(
        r#"
        SELECT id, name, description, leader_id, level, created_at
        FROM guilds
...
/// Create a new guild
pub async fn create_guild(pool: &DbPool, data: &CreateGuild) -> Result<Guild, sqlx::Error> {
    let now = Utc::now();
    let id = Uuid::new_v4();
...
/// Leave a guild
pub async fn leave_guild(pool: &DbPool, character_id: Uuid) -> Result<(), sqlx::Error> {
    sqlx::query(r#"DELETE FROM guild_members WHERE character_id = $1"#)
        .bind(character_id)
        .execute(pool)
        .await?;
...
pub async fn is_guild_name_taken(pool: &DbPool, name: &str) -> Result<bool, sqlx::Error> {
    let count: (i64,) =
        sqlx::query_as(r#"SELECT COUNT(*) FROM guilds WHERE LOWER(name) = LOWER($1)"#)
            .bind(name)
            .fetch_one(pool)
            .await?;
